...
describe("proxy/dataUrl · buildDataUrl", () => {
  it("appends the endpoint to a bare-origin base", () => {
    ...
    expect(url).toBe("https://proxy.example.test/api/v1/proxy-wfs?q=abc-DEF_123");
  });
  ...
  it("PRESERVES an ingress path prefix on the base (P2 fix)", () => {
    ...
    expect(url).toBe("https://example.test/published/proxy/api/v1/proxy-wfs?q=abc-DEF_123");
  });
