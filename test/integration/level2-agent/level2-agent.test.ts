...
const mcpScenarios = [
  ...
  },
  {
    ...
    toolMode: "mcp",
    requiredToolCalls: ["geocode", "gpf_search_types", "gpf_describe_type", "gpf_get_features", "distance"],
    expectedResponseFragments: ["Sivom", "Mondeville"],
    assertScenarioResult: ({ normalizedFinalMessage }) => {
      expect(containsNumberInRange(normalizedFinalMessage, 25, 35)).toBe(true);
    },
  }
] satisfies Level2AgentScenario[];
