...
| `PROXY_UPSTREAM_TIMEOUT`          | Délai (secondes) des appels amont du proxy (WFS, isochrone et itinéraire), plus court que `HTTP_TIMEOUT` pour qu'une requête à 2 appels (`intersects_feature_filter` ou `isoline_filter`) reste sous le délai du navigateur/Carto.                                                                                                                                            | `10`                                             |
...
- [Configuration d'un proxy d'entreprise](config/corporate-proxy.md)
