 ...
 *
 * @param property Property metadata from the embedded catalog.
 * @returns `true` when the property format indicates a date-like value.
 */
function isDateProperty(property: OgcCollectionProperty) {
  return property.type == "string" && (property.format ?? "").startsWith("date")
}
 ...
 */
function ensureNumericOrDateProperty(property: OgcCollectionProperty, operator: keyof typeof NUMERIC_COMPARISON_OPERATORS, propertyName: string) {
...
function coerceScalarValueForProperty(property: OgcCollectionProperty, value: string, propertyName: string) {
  ...
  if (enumValues.length > 0 && !enumValues.includes(value)) {
    throw new Error(`La propriété '${propertyName}' exige une valeur parmi : ${enumValues.join(", ")}. Utilise \`gpf_describe_type\` pour avoir une description détaillée de ces différentes valeurs possibles et pour mieux comprendre les champs du type.`);
  }
  return value;
}
...
function coerceOrderedValueForProperty(property: OgcCollectionProperty, value: string, operator: keyof typeof NUMERIC_COMPARISON_OPERATORS, propertyName: string) {
  ensureNumericOrDateProperty(property, operator, propertyName);
  if (isDateProperty(property)) {
