...
const isosurfaceCostSchema = z.object({
  profile: navigationProfileSchema,
  cost_type: isosurfaceCostTypeSchema,
  cost_value: isosurfaceCostValueSchema,
}).strict();

const isosurfaceFilterSchema = isosurfacePointSchema
  .merge(isosurfaceCostSchema)
...
const gpfSpatialFilterInputSchema = z.object({
  ...
  isosurface_filter: isosurfaceFilterSchema
    .optional()
...
export type SpatialFilter =
  ...
  | ({ operator: "isosurface" } & z.infer<typeof isosurfaceFilterSchema>);
...
}).merge(isosurfaceCostSchema).strict();
