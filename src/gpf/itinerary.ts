...
export class NavigationItineraryLayerClient {
  ...
  async getItineraryWithGeometry(input: ItineraryGeometryInput): Promise<ItineraryWithGeometryResult> {
    ...
    if (!(isGeoJsonGeometryLike(result.geometry) && result.geometry.type == "LineString")) {
      throw new Error("Le service d'itinéraire n'a pas renvoyé de LineString exploitable.");
    }
    return {
