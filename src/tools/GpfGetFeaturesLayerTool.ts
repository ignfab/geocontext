 ...
 * transport (so `tools/list` is identical everywhere), but fails fast with a clear
 * FR error when called under stdio — before any WFS work or token minting.
 */
...
class GpfGetFeaturesLayerTool extends BaseTool<GpfGetFeaturesLayerInput> {
  ...
  async execute(input: GpfGetFeaturesLayerInput) {
    ...
    if (env.TRANSPORT_TYPE !== "http") {
      throw new Error(
        "`gpf_get_features_layer` n'est disponible qu'en mode http (le service qui sert la couche est un proxy http). En local (stdio), utiliser `gpf_get_features`.",
      );
    }
